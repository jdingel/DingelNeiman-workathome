...
//Load state occupation counts
import excel using "../input/state_M2018_dl.xlsx", clear firstrow
...
collapse (sum) TOT_EMP (mean) H_MEAN (firstnm) ST STATE, by(AREA BroadGroupCode)
...
keep AREA STATE teleworkable_emp teleworkable_wage MSA_totalemployment
duplicates drop
egen tag = tag(AREA)
assert tag==1
save "../output/state_2018_teleworkable_manual.dta", replace
...
egen tag = tag(AREA)
assert tag==1
save "../output/state_2018_teleworkable_onet.dta", replace

use AREA STATE teleworkable* using "../output/state_2018_teleworkable_manual.dta", clear
rename (teleworkable_emp teleworkable_wage) (teleworkable_manual_emp teleworkable_manual_wage )
merge 1:1 AREA using "../output/state_2018_teleworkable_onet.dta", update keepusing(teleworkable*) assert(match) nogen
keep AREA STATE teleworkable*
export delimited using "../output/state_workfromhome.csv", replace
